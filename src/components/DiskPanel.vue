...
function toggleDevice(dev) {
  expandedDevices.value = { ...expandedDevices.value, [dev]: !expandedDevices.value[dev] }
}

function toggleRaw(dev) {
  showRawSmart.value = { ...showRawSmart.value, [dev]: !showRawSmart.value[dev] }
}
    ...
    <div class="card wide">
      <div class="card-header">
        ...
        <button class="refresh-btn" :class="{ spinning: metrics.loading.disk || metrics.loading.smart }"
                @click="() => { metrics.refreshField('disk'); metrics.refreshField('smart') }" title="Refresh">↻</button>
      </div>
      ...
      <div class="section" v-if="metrics.disk?.filesystems?.length">
        ...
        <div class="table-wrap">
          <table>
            <thead>
              <tr><th>Mount</th><th>Type</th><th>Size</th><th>Used</th><th>Avail</th><th>Use%</th></tr>
            </thead>
            <tbody>
                ...
                <td>
                  <div class="usage-cell">
                    <div class="progress-bar small">
                      <div class="progress-fill" :style="{ width: fs.usePercent + '%', background: usageColor(fs.usePercent) }"></div>
                    </div>
          ...
          <div class="smart-device-card" v-for="dev in metrics.smart.devices" :key="dev.device">
            <div class="smart-device-header" @click="toggleDevice(dev.device)">
              ...
              <div class="smart-device-badges">
                <span class="health-badge" :class="dev.health === 'PASSED' ? 'passed' : 'failed'">
                  {{ dev.health }}
                </span>
            ...
            <div v-if="expandedDevices[dev.device]" class="smart-details">
              ...
              <div class="table-wrap" v-if="dev.attributes?.length">
                <table class="smart-table">
                  <thead>
                    <tr><th>ID</th><th>Attribute</th><th>Value</th><th>Worst</th><th>Thresh</th><th>Raw</th><th>Status</th></tr>
                  </thead>
                  <tbody>
                    <tr v-for="attr in dev.attributes" :key="attr.id" :class="{ 'attr-warning': attr.status === 'warning' }">
                      ...
                      <td>
                        ...
                        <div class="attr-explain">{{ attr.explanation }}</div>
                      </td>
                      <td class="mono">{{ attr.value }}</td>
                      <td class="mono">{{ attr.worst }}</td>
                      <td class="mono">{{ attr.threshold }}</td>
                      <td class="mono">{{ attr.rawValue }}</td>
                      <td>
                        <span class="status-dot" :class="attr.status"></span>
                      </td>
                    </tr>
                  </tbody>
                </table>
      ...
      <div class="card-placeholder" v-if="!metrics.disk">Loading Disk data…</div>
    </div>
  </div>
</template>
